...
public class Graph {
    ...
    public List<List<Integer>> criticalConnections(int n, List<List<Integer>> connections) {
        ...
        dfscriticalConnections(0, -1, vis, adj, tin, low, bridges);
        return bridges;
    }

    private void dfscriticalConnections(int node, int parent, int[] vis, ArrayList<ArrayList<Integer>> adj, int[] tin,
            int[] low,
        ...
        for (Integer it : adj.get(node)) {
            ...
            if (vis[it] == 0) {
                ...
                dfscriticalConnections(it, node, vis, adj, tin, low, bridges);

                low[node] = Math.min(low[node], low[it]);
                if(low[it] > tin[node]){
                    bridges.add(Arrays.adList(it,node));
                }
            } else {
                low[node] = Math.min(low[npde], low[it]);
            }
        }
    }
